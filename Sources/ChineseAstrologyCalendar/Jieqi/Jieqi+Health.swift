...
extension Jieqi {
    ...
    public var season: Season {
        switch self {
        case .startOfSpring, .rainWater, .awakeningOfInsects, .springEquinox, .clearAndBright, .grainRain:
            return .spring
        case .startOfSummer, .grainBuds, .grainInEar, .summerSolstice, .minorHeat, .majorHeat:
            return .summer
        case .startOfAutumn, .endOfHeat, .whiteDew, .autumnEquinox, .coldDew, .frostDescent:
            return .fall
        case .startOfWinter, .minorSnow, .majorSnow, .winterSolstice, .minorCold, .majorCold:
            return .winter
        }
    }
